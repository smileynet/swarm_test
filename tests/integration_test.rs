...
fn test_full_session_lifecycle() {
    ...
    let session = tmux_session::new_session(&session_name).expect("Failed to create session");
    ...
    let retrieved_session =
        tmux_session::get_session(&session.id).expect("Failed to retrieve session");

    assert_eq!(session.id, retrieved_session.id);
    assert_eq!(session.name, retrieved_session.name);

    let all_sessions = tmux_session::list_sessions().expect("Failed to list sessions");

    let found = all_sessions.iter().any(|s| s.id == session.id);
    ...
    if rename_result.is_ok() {
        ...
        if let Ok(all_sessions) = tmux_session::list_sessions() {
            if let Some(renamed_session) = all_sessions.iter().find(|s| s.id == session_id) {
                assert_eq!(
                    renamed_session.name, new_name,
                    "Session name should be updated to {}",
                    new_name
                );
            }
        }
        ...
        eprintln!(
            "Warning: Session rename not supported in this environment: {:?}",
            rename_result
        );
    }
    ...
    if kill_result.is_err() {
        eprintln!(
            "Warning: Failed to kill session {}, trying cleanup: {:?}",
            session.id.0, kill_result
        );
    }
...
fn test_message_sending_workflow() {
    ...
    sender
        .send_prompt(&pane_id, test_message)
    ...
    let read_message = sender.read_prompt(&pane_id).expect("Failed to read prompt");

    assert_eq!(read_message.trim(), test_message);

    sender
        .clear_prompt(&pane_id)
...
fn test_message_with_metadata() {
    ...
    sender
        .send_prompt_with_metadata(&pane_id, test_message, &metadata)
...
fn test_log_reading_workflow() {
    ...
    let log_content = reader.read_log(&session_id).expect("Failed to read log");
    ...
    let log_lines = reader
        .read_log_lines(&session_id)
        .expect("Failed to read log lines");

    assert_eq!(log_lines.len(), test_lines.len());

    let last_n = reader.tail_log(&session_id, 2).expect("Failed to tail log");
    ...
    let search_results = reader
        .search_log(&session_id, "Processing")
    ...
    let log_size = reader
        .get_log_size(&session_id)
        .expect("Failed to get log size");

    assert!(log_size > 0);

    reader.clear_log(&session_id).expect("Failed to clear log");

    let cleared_content = reader
        .read_log(&session_id)
...
fn test_session_list_empty() {
    ...
    let sessions = tmux_session::list_sessions().expect("Failed to list sessions");

    let test_sessions: Vec<_> = sessions
        .iter()
        .filter(|s| s.name.starts_with(TEST_SESSION_PREFIX))
        .collect();

    assert!(
        test_sessions.is_empty(),
        "No test sessions should exist after cleanup. Found: {:?}",
        test_sessions
    );
}
...
fn test_multiple_sessions() {
    ...
    let session1 =
        tmux_session::new_session(&session1_name).expect("Failed to create first session");
    let session2 =
        tmux_session::new_session(&session2_name).expect("Failed to create second session");
