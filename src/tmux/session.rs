...
pub fn new_session(name: impl Into<String>) -> Result<Session> {
    ...
    if !response.success {
        return Err(TmuxError::Command(
            response.error.unwrap_or_else(|| "Failed to create session".to_string()),
        ));
    }
...
pub fn kill_session(id: &SessionId) -> Result<()> {
    ...
    if !response.success {
        return Err(TmuxError::Command(
            response.error.unwrap_or_else(|| "Failed to kill session".to_string()),
        ));
    }
...
pub fn list_sessions() -> Result<Vec<Session>> {
    let cmd = Command {
        ...
        target: CommandTarget::Server,
        args: vec!["-F".to_string(), "#{session_id}:#{session_name}:#{session_attached}".to_string()],
    };
    ...
    if !response.success {
        return Err(TmuxError::Command(
            response.error.unwrap_or_else(|| "Failed to list sessions".to_string()),
        ));
    }
...
pub fn attach_session(id: &SessionId) -> Result<()> {
    ...
    if !response.success {
        return Err(TmuxError::Command(
            response.error.unwrap_or_else(|| "Failed to attach session".to_string()),
        ));
    }
...
pub fn detach_session(id: &SessionId) -> Result<()> {
    ...
    if !response.success {
        return Err(TmuxError::Command(
            response.error.unwrap_or_else(|| "Failed to detach session".to_string()),
        ));
    }
...
pub fn rename_session(id: &SessionId, new_name: impl Into<String>) -> Result<()> {
    ...
    if !response.success {
        return Err(TmuxError::Command(
            response.error.unwrap_or_else(|| "Failed to rename session".to_string()),
        ));
    }
